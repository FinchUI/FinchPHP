...
│   │   ├── ThemeAdmin.php      # 🔜 主题管理：切换/预览/自定义（当前在 ExtensionAdmin 内）
│   │   ├── AiAdmin.php         # 🔜 AI 大模型设置（当前在 SettingAdmin 内）
...
| Model 缺失 | `PostCategory`、`PostTag`、`RememberToken`、`PostMeta`、`UserMeta` | 未创建独立模型文件 | 关联表和 meta 表当前通过 Query Builder 直接操作，后续 ORM 关联完善时补齐 |
...
- [ ] `PostCategory` / `PostTag` / `RememberToken` / `PostMeta` / `UserMeta` 独立模型补齐
- [ ] 模型关联方法（belongsTo / hasMany / belongsToMany）
...
- [ ] 模块管理（`ModuleAdmin.php`：侧栏模块增删改与排序）
- [ ] 主题管理独立页面（`ThemeAdmin.php`：切换/预览/自定义）
- [ ] AI 大模型设置独立页面（`AiAdmin.php`）
- [ ] 功能模块管理（`ModuleFeatureAdmin.php`）
