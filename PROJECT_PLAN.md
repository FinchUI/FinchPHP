...
> **图例**：✅ 已落地 | 🔲 占位/空壳（仅有骨架或空文件） | 🔜 尚未创建（规划中有，代码库中不存在） | 📦 安装时生成
>
> 以下目录树反映代码库实际状态（截至 2026-06-13），与「规划中有但未落地的文件」以 🔜 标注，方便追踪开发进度。
...
│   ├── nginx-uploads.conf.example # ✅ Nginx 上传目录安全配置示例
│   ├── stage20_verify.sh       # ✅ 安装后验证脚本
│   ├── API.md                  # ✅ API 文档
│   ├── DEPLOY.md               # ✅ 部署文档
│   ├── RELEASE_HANDOFF_*.md    # ✅ 发布交接文档
│   └── WORKSPACE_SMOKE_LOG_*.md # ✅ 冒烟测试日志
├── scripts/                    # 运维脚本目录
...
│   │   ├── css/                # 🔲 空目录（后台框架样式改由预装 tabler 插件提供）
│   │   └── img/                # 🔲 系统公共图片、图标、默认资源（仅有占位 index.html）
│   ├── Core/                   # 框架基础设施层
...
│   │   ├── Settings.php        # ✅ 运行期设置管理：读取/缓存系统/主题/插件/模块配置
│   │   ├── Installer.php       # ✅ 安装器：执行迁移、写入种子数据
│   │   ├── Gate.php            # ✅ 权限校验门面：$user->can('edit_post') 能力检查
│   │   ├── Validation.php      # ✅ 验证入口：声明式规则校验代理（实际逻辑在 Validation/Validator.php）
│   │   └── HtmlCleaner.php     # ✅ HTML 清洗器：按白名单过滤 Quill 输出，去除 XSS 风险
│   ├── Validation/             # 验证器实现
│   │   └── Validator.php       # ✅ 声明式规则校验器（required/max/email/unique 等）
...
│   │   ├── PostService.php     # ✅ 文章读取、列表查询、前端展示
│   │   ├── PostManageService.php # ✅ 文章保存、发布、分类/标签关联维护（后台写入逻辑）
...
│   │   ├── MailService.php     # ✅ 邮件门面：找回密码/验证/通知调度 + PHP mail() 保底
│   │   ├── SmsService.php      # ✅ 短信门面：验证码生成/校验/限流
│   │   ├── CaptchaService.php  # ✅ 验证码门面：校验调度
│   │   ├── SearchService.php   # ✅ 搜索门面：查询调度
│   │   ├── AiService.php       # ✅ AI 门面：调用调度/限流/日志
│   │   ├── SocialLoginService.php # ✅ 社会化登录门面：授权/绑定/建号
│   │   ├── CommentService.php  # ✅ 评论读取、列表、前端展示
│   │   ├── CommentManageService.php # ✅ 评论审核/删除/批量操作（后台写入逻辑）
│   │   ├── TaxonomyService.php # ✅ 分类/标签读取与前端展示
│   │   ├── TaxonomyManageService.php # ✅ 分类/标签创建/编辑/合并/清理（后台写入逻辑）
│   │   └── ProviderRuntime.php # ✅ Provider 运行时：注册/调度各 Provider 插件
...
│   │   ├── ApiToken.php        # ✅ API Token 模型
│   │   ├── UserSocialAccount.php # ✅ 第三方登录账号绑定模型
│   │   ├── SmsCode.php         # ✅ 手机验证码模型
│   │   ├── Module.php          # ✅ 侧栏模块/组件模型
│   │   ├── Upload.php          # ✅ 上传文件模型
│   │   ├── SystemSetting.php   # ✅ 系统运行期配置模型
│   │   ├── ThemeSetting.php    # ✅ 主题配置模型
│   │   ├── PluginSetting.php   # ✅ 插件配置模型
│   │   ├── ModuleSetting.php   # ✅ 官方模块配置模型
│   │   ├── AiModel.php         # ✅ AI 大模型配置模型
│   │   ├── AiLog.php           # ✅ AI 调用摘要日志模型
│   │   ├── Migration.php       # ✅ 数据库迁移记录模型
│   │   ├── RememberToken.php   # ✅ 记住我长期登录 Token 模型
│   │   ├── PostMeta.php        # ✅ 文章低频扩展字段模型
│   │   └── UserMeta.php        # ✅ 用户低频扩展字段模型
│   ├── Database/               # 数据库层
│   │   ├── Driver.php          # ✅ 数据库驱动接口：定义所有驱动必须实现的方法
│   │   ├── AbstractDriver.php   # ✅ 数据库驱动抽象基类：公共 PDO 封装逻辑
...
│   │   ├── Schema/             # 数据库 Schema 构建器（PHP 代码定义，自动生成 MySQL/SQLite DDL）
│   │   │   ├── Blueprint.php   # ✅ 表结构蓝图：定义列、索引、约束
│   │   │   ├── Builder.php     # ✅ Schema 构建器：创建/修改表结构
│   │   │   └── Column.php      # ✅ 列定义：类型、长度、默认值、可空等
│   │   └── Migrations/         # 版本化迁移脚本（PHP 类，升级时逐个执行）
│   │       ├── 20260101000000_create_content_tables.php # ✅ 内容核心表迁移
│   │       ├── 20260101000100_create_user_tables.php     # ✅ 用户与权限表迁移
│   │       ├── 20260101000200_create_system_tables.php   # ✅ 系统与扩展表迁移
│   │       └── 20260101000300_create_meta_ai_tables.php  # ✅ Meta 与 AI 表迁移
│   ├── Middleware/              # 中间件层
│   │   ├── MiddlewareInterface.php # ✅ 中间件接口定义
│   │   ├── AuthMiddleware.php   # ✅ 登录态校验
│   │   ├── RoleMiddleware.php   # ✅ 角色权限校验
│   │   ├── CsrfMiddleware.php   # ✅ CSRF 令牌校验
│   │   ├── CorsMiddleware.php   # ✅ 跨域头处理
│   │   ├── ApiAuthMiddleware.php # ✅ API Bearer Token 认证
│   │   ├── ApiAbilityMiddleware.php # ✅ API Token 能力范围校验
│   │   └── ThrottleMiddleware.php # 🔜 频率限制（防刷，1.1 实现）
│   ├── Admin/                  # 后台管理类（经 Router 分发，命名空间 Finch\Admin）
│   │   ├── AdminLayout.php     # ✅ 后台布局渲染：侧栏、顶栏、面包屑、页面骨架
│   │   ├── AuthAdmin.php       # ✅ 后台登录/登出
│   │   ├── DashboardAdmin.php  # ✅ 仪表盘：网站统计概览、快速入口
│   │   ├── PostAdmin.php       # ✅ 文章编辑器：撰写/编辑/管理文章
...
│   │   ├── ExtensionAdmin.php  # ✅ 扩展管理：插件/主题/模块统一管理入口
│   │   ├── TokenAdmin.php      # ✅ API Token 管理：创建/撤销/能力设置
│   │   ├── ModuleAdmin.php     # ✅ 模块管理：侧栏模块的增删改与排序
│   │   ├── ThemeAdmin.php      # ✅ 主题管理：切换/预览/自定义
│   │   ├── AiAdmin.php         # ✅ AI 大模型设置
│   │   └── ModuleFeatureAdmin.php # ✅ 功能模块管理：官方模块启用/禁用
│   ├── Api/                    # API 接口模块（JSON 响应，供第三方/移动端调用）
│   │   ├── ApiResource.php     # ✅ API 统一响应封装：{code, message, data, meta}
│   │   └── V1/                 # v1 版本 API 端点
│   │       ├── PostsApi.php    # ✅ 文章 API：列表/详情/创建/更新/删除
│   │       ├── CategoriesApi.php # ✅ 分类 API：分类树、列表、详情、CRUD
│   │       ├── TagsApi.php     # ✅ 标签 API：标签列表、详情、CRUD
│   │       ├── CommentsApi.php # ✅ 评论 API：评论的提交/审核/删除
│   │       ├── UserApi.php     # ✅ 用户 API：用户信息、登录认证
│   │       ├── UploadsApi.php  # ✅ 上传 API：文件上传与删除
│   │       ├── SystemApi.php   # ✅ 系统 API：站点信息、设置读写
│   │       └── ModulesApi.php  # ✅ 模块 API：侧栏模块的管理
│   ├── Helper/                 # 辅助函数集合（按职责拆分，避免单文件臃肿）
│   │   ├── TemplateTags.php    # ✅ 模板函数标签：title() / content() / permalink() / the_post_thumbnail() 等
│   │   ├── Security.php        # ✅ 安全过滤：XSS 清洗 / SQL 转义 / CSRF 令牌 / 脱敏工具
│   │   └── Utility.php         # ✅ 通用工具：URL 构建 / 文件操作 / 日期格式化 / slug 生成
│   ├── Languages/              # 系统语言包（后台管理 + 前端公共字符串）
│   │   ├── zh-cn.php           # ✅ 简体中文 ✅ 1.0
│   │   ├── zh-tw.php           # ✅ 繁体中文 ✅ 1.0
│   │   ├── en.php              # ✅ 英文     ✅ 1.0
│   │   ├── de.php              # 🔜 德语
...
│   │   │   └── assets/
│   │   │       ├── css/
│   │   │       │   ├── tabler.min.css         # ✅ Tabler 核心样式
│   │   │       │   ├── tabler-icons.min.css   # ✅ Tabler 图标样式
│   │   │       │   ├── admin.css              # ✅ 后台样式扩展（本地覆盖层）
│   │   │       │   └── fonts/
│   │   │       │       ├── tabler-icons.woff2 # ✅
│   │   │       │       ├── tabler-icons.woff  # ✅
│   │   │       │       └── tabler-icons.ttf   # ✅
│   │   │       └── js/
│   │   │           ├── tabler.min.js          # ✅ Tabler 核心 JS
│   │   │           └── admin.js               # ✅ 后台交互增强脚本（如移动端表格包裹）
│   │   ├── quill/              # ✅ 预装·不可删除：编辑器提供者，提供 Quill 富文本编辑器
│   │   │   ├── include.php     # ✅ 注册为编辑器提供者
│   │   │   ├── script/
│   │   │   │   └── quill.min.js # 🔜 Quill 编辑器脚本（待内置）
│   │   │   └── assets/
│   │   │       └── css/
│   │   │           └── quill.snow.css # 🔜 Quill 雪景主题样式（待内置）
│   │   ├── social-github/      # ✅ 预装：GitHub 社会化登录
│   │   │   ├── include.php     # ✅
│   │   │   └── plugin.json     # ✅
│   │   ├── notify-sms/         # ✅ 预装：短信通知插件
│   │   │   ├── include.php     # ✅
│   │   │   └── plugin.json     # ✅
│   │   ├── notify-email/       # ✅ 预装：邮件通知插件
│   │   │   ├── include.php     # ✅
│   │   │   └── plugin.json     # ✅
│   │   ├── captcha-gd/         # ✅ 预装：GD 图形验证码
│   │   │   ├── include.php     # ✅
│   │   │   └── plugin.json     # ✅
│   │   ├── search-like/        # ✅ 预装：SQL LIKE 搜索
│   │   │   ├── include.php     # ✅
│   │   │   └── plugin.json     # ✅
│   │   ├── ai-openai/          # ✅ 预装：OpenAI 协议 AI 适配
│   │   │   ├── include.php     # ✅
│   │   │   └── plugin.json     # ✅
│   │   └── {plugin_name}/      # 第三方插件（目录名即插件 ID）
│   │       ├── include.php     #   插件入口：定义插件元信息（名称/版本/作者），注册钩子
│   │       ├── plugin.json     #   插件元信息（JSON 格式，供 PluginService 扫描）
│   │       ├── include/        #   插件函数库（按用途分文件，由 include.php 加载）
...
│   │   └── blog/               # 🔲 博客模块（1.0 示例模块，仅含 module.json）
│   │       └── module.json     # ✅ 模块元信息
...
│   │       │   └── css/
│   │       │       └── theme.css # ✅ 主题样式表
│   │       └── languages/      # 🔜 主题自带语言包（前端翻译，优先级高于系统语言包）
│   │           ├── zh-cn.php   #   简体中文
│   │           ├── zh-tw.php   #   繁体中文
│   │           └── en.php      #   英文
│   ├── uploads/                # 用户上传文件（按年月子目录组织）
│   │   ├── .htaccess           # ✅ Apache 禁止执行 PHP
│   │   ├── 2026/06/            # 🔲 上传目录（按年月归档）
...
| 差异点 | 原规划 | 实际落地 | 原因 |
| ------ | ------ | ------ | ------ |
| 数据库 Schema | `Schema/mysql.sql` + `Schema/sqlite.sql` | `Schema/Blueprint.php` + `Builder.php` + `Column.php` + PHP 迁移类 | 用 PHP 代码定义 Schema，由 Migrator 自动生成对应数据库的 DDL，避免维护两份 SQL |
| 迁移文件命名 | `Migrations/1.0.0.php` 单文件 | `Migrations/20260101*_create_*.php` 多文件 | 采用时间戳前缀命名，支持增量迁移与插件独立迁移 |
| API 目录结构 | `Api/PostApi.php` 扁平结构 | `Api/V1/PostsApi.php` 版本化子目录 | 从设计之初即按 RESTful 版本化组织，为后续 `/v2/` 并行预留 |
| 验证器位置 | `Core/Validation.php` 单文件 | `Core/Validation.php`（入口）+ `Validation/Validator.php`（实现） | 验证逻辑复杂度高，独立目录便于扩展自定义规则 |
| Admin 额外文件 | 无 | `AdminLayout.php`、`AuthAdmin.php`、`ExtensionAdmin.php`、`TokenAdmin.php` | 后台布局渲染、登录页、扩展统一管理、Token 管理独立为 Admin 类 |
| Service 拆分 | `PostService` 单文件 | `PostService`（读）+ `PostManageService`（写） | 读写分离：前台读逻辑与后台写逻辑复杂度差异大，拆分避免单文件膨胀 |
| 数据库驱动 | `Driver.php` 接口 + `Mysql.php`/`Sqlite.php` | 增加 `AbstractDriver.php` 抽象基类 | 提取两驱动的公共 PDO 封装逻辑，减少重复代码 |
| Middleware | 规划 4 个 | 实际 7 个，增加 `MiddlewareInterface`、`ApiAuthMiddleware`、`ApiAbilityMiddleware` | API Token 认证与能力校验独立为中间件，遵循单一职责 |
| Model 缺失 | `PostCategory`、`PostTag`、`RememberToken`、`PostMeta`、`UserMeta` | ✅ 已创建独立模型文件，含关联同步、类型转换、Token 续期/清理等方法 | 之前通过 Query Builder 直接操作，现已补齐 ORM 层 |
...
**内置 API 模块**（位于 `system/Api/V1/`，统一由 `ApiResource` 封装响应）：

- `PostsApi` — 文章 CRUD + 列表 + 分页
- `CategoriesApi` — 分类树、分类 CRUD、分类文章列表
- `TagsApi` — 标签 CRUD、标签合并、标签文章列表
- `CommentsApi` — 评论 CRUD + 列表
- `UserApi` — 用户信息 + 登录认证
- `UploadsApi` — 文件上传/删除
- `SystemApi` — 系统信息/设置
- `ModulesApi` — 🔜 模块管理
...
| `CsrfMiddleware` | CSRF 令牌校验（表单 + AJAX 双模式） | ✅ |
| `ApiAuthMiddleware` | API Bearer Token 认证 | ✅ |
| `ApiAbilityMiddleware` | API Token 能力范围校验 | ✅ |
...
- [x] 目录结构创建（system/ content/ storage/ install/）
- [x] `system/bootstrap.php` 启动引导
- [x] `config.php` 安装级配置加载与生成
- [x] `Finch\App` 单例类
- [x] `Finch\Core\Request` / `Finch\Core\Response` 请求响应基础类
- [x] `Finch\` 根命名空间 + PSR-4 风格自动加载器（自实现，不依赖 Composer）
- [x] 常量定义（FP_PATH / FP_CONTENT_DIR / FP_STORAGE_DIR）
- [x] 命名空间映射：`Finch\` => `system/`
- [x] `Config` 类读取数组式 `config.php`，业务代码禁止直接 include 配置文件，运行期设置交给 `SettingService`
- [x] `Logger` / `ErrorHandler` / `Session` 基础类
- [x] 开发模式与生产模式的错误输出和日志策略
- [x] Controller / Service / Model 三层边界落实
- [x] `Finch\Core\Settings` 运行期设置管理
- [x] `Finch\Core\Gate` 权限校验门面
- [x] `Finch\Core\Installer` 安装器
- [x] `Finch\Core\Validation` + `Finch\Validation\Validator` 输入验证

### 阶段 2：数据库层

- [x] `Finch\Database\Driver` 接口
- [x] `Finch\Database\AbstractDriver` 抽象基类（提取公共 PDO 封装逻辑）
- [x] 2 种 PDO 驱动实现（Mysql / Sqlite）
- [x] `Finch\Database\Query` SQL 构建器
- [x] `Migrator` 数据库迁移执行器
- [x] `migration` 表与初始版本记录
- [x] 初始 migration（4 个 PHP 迁移类，替代原规划的 SQL 文件）
- [x] Schema 构建器：`Blueprint` / `Builder` / `Column`（PHP 代码定义，自动生成 DDL）
- [x] `system_setting` / `theme_setting` / `plugin_setting` / `module_setting` 独立配置表
- [x] `ai_model` 多平台大模型配置表与 Key 加密字段
- [x] `ai_log` 调用摘要日志表与清理索引
...
- [x] `Finch\Model\BaseModel` 基类
- [x] 内容、用户、系统核心 Model 类实现（Post / Category / Tag / Comment / User / Role / Upload / Module / Migration 等）
- [x] `SystemSetting` / `ThemeSetting` / `PluginSetting` / `ModuleSetting` / `AiModel` / `AiLog` 模型实现
- [x] `SettingService` 类型转换、autoload 缓存、secret 加密读取
- [x] 基本 CRUD 操作
- [x] Controller / Service / Model 职责边界落实
- [x] 读写分离：`PostService`（前台读）+ `PostManageService`（后台写）；`CommentService` + `CommentManageService`；`TaxonomyService` + `TaxonomyManageService`
- [ ] 字段 cast、时间格式、软删除、批量赋值白名单（`$casts` + `$fillable` 已添加到 BaseModel 和 Post，其余 Model 待逐一完善）
- [ ] 分类/标签关联保存与 `post_count` 事务维护
- [x] `PostCategory` / `PostTag` / `RememberToken` / `PostMeta` / `UserMeta` 独立模型补齐
- [x] 模型关联方法（belongsTo / hasMany / belongsToMany）
...
- [x] 关键钩子点铺设
- [x] 插件元信息格式与兼容性检查
- [x] 插件默认配置页、顶部菜单/左侧栏/系统设置/隐藏入口注册规则
- [x] `ProviderRuntime` 运行时：注册/调度各 Provider 插件
- [ ] install / activate / deactivate / uninstall 生命周期完整实现
- [ ] Hook 参数、返回值、中断信号、优先级文档

### 阶段 5：路由 + 视图

- [x] `Finch\Core\Router` 三级路由
- [x] 6 个默认控制器动作（index/single/page/category/tag/search）
- [x] route name 唯一规则与 URL 生成器
- [x] URL 规则生成
- [ ] 404、分页、预览、slug 冲突处理规则完善
- [ ] 动态 URL 与伪静态 URL 的互相生成与回退
- [ ] Apache/Nginx 伪静态示例规则

### 阶段 6：模板系统

- [x] 模板加载器（`Finch\Core\Template`）
- [x] 模板上下文栈（`Finch\Core\TemplateContext`）
- [x] 模板函数标签库（`Finch\Helper\TemplateTags`）
- [x] 默认主题模板（`content/themes/default/template/`）
- [x] 系统兜底主题（`system/Theme/`）
- [ ] 模板变量作用域与输出转义规则
- [ ] `the_post_thumbnail()` 本地/远程图片安全约束
- [ ] 主题 `theme.json`、截图、语言包、模板优先级规则
- [ ] 主题 `include.php` 与 `include/` 函数库机制

### 阶段 7：后台管理

- [x] 后台布局渲染（`AdminLayout.php`）
- [x] 后台登录/登出（`AuthAdmin.php`）
- [x] 仪表盘（`DashboardAdmin.php`）
- [x] 文章管理（`PostAdmin.php`）
- [x] 页面管理（`PageAdmin.php`）
- [x] 分类管理（`CategoryAdmin.php`）
- [x] 标签管理（`TagAdmin.php`）
- [x] 评论管理（`CommentAdmin.php`）
- [x] 用户管理（`UserAdmin.php`）
- [x] 文件管理（`UploadAdmin.php`）
- [x] 系统设置（`SettingAdmin.php`）
- [x] 扩展管理（`ExtensionAdmin.php`：插件/主题/模块统一入口）
- [x] API Token 管理（`TokenAdmin.php`）
- [x] 后台权限控制（AuthMiddleware + RoleMiddleware）
- [x] CSRF 中间件（`CsrfMiddleware.php`）
- [x] 模块管理（`ModuleAdmin.php`：侧栏模块增删改与排序）
- [x] 主题管理独立页面（`ThemeAdmin.php`：切换/预览/自定义）
- [x] AI 大模型设置独立页面（`AiAdmin.php`）
- [x] 功能模块管理（`ModuleFeatureAdmin.php`）
- [ ] 用户中心：手机验证码 + GitHub OAuth + 账号绑定页
- [ ] 伪静态设置：预设规则 + 自定义 URL
...
- [x] API 入口 + Token 认证（`ApiAuthMiddleware` + `ApiAbilityMiddleware`）
- [x] API 统一响应封装（`ApiResource`）
- [x] 版本化目录结构（`Api/V1/`）
- [x] 7 个内置 API 端点（Posts / Categories / Tags / Comments / User / Uploads / System）
- [x] CORS 中间件（`CorsMiddleware.php`）
- [x] 中间件管道（Auth / Role / CSRF / CORS）
- [x] 输入验证层（声明式规则校验）
- [x] API Token hash 存储、能力范围
- [x] 模块 API（`ModulesApi.php`）
- [ ] 社会化登录 API 扩展点
- [ ] HTTP Method、状态码、错误码、分页参数、排序/筛选参数契约完善
- [ ] API Token 过期、撤销、最后使用时间
- [ ] CORS 白名单、Token 权限与频率限制预留

### 阶段 9：功能模块框架

- [x] 模块注册/激活基础（`ModuleService.php`）
- [x] 模块元信息（`content/modules/blog/module.json` 示例）
- [ ] 模块自带模板自动加载
- [ ] 安装向导中可选模块预选启用
- [ ] 模块依赖、最低版本、启用/停用生命周期
- [ ] 模块自带 migration 与独立数据表规则
- [x] 模块与插件共用接口但后台管理入口分离（`ModuleFeatureAdmin.php` 独立于 `ExtensionAdmin.php`）

### 阶段 10：安装向导

- [x] 环境检测（PHP 版本、扩展、目录权限）
- [x] 数据库配置（MySQL / SQLite 选择 + 连接信息）
- [x] 初始化安装（执行迁移、写入种子数据）
- [x] 写入安装级 `config.php`
- [x] 必需 PHP 扩展与目录权限检测
- [x] 安装完成写入 install lock，防止重复安装
- [x] 三语言安装向导（zh-cn / zh-tw / en）
- [ ] `config.php` 写入失败时的手动复制方案
- [ ] 后台可改配置初始化到数据库设置表

### 阶段 11：补充安全、缓存、优化

- [x] CSRF 防护（`CsrfMiddleware.php`）
- [x] 文件缓存系统（`Finch\Core\Cache`）
- [x] 上传目录禁止执行 PHP 的 Apache/Nginx 示例（`.htaccess` + `nginx-uploads.conf.example`）
- [x] 缩略图处理（`Finch\Core\Thumbnail`）
- [x] XSS 全面覆盖（`HtmlCleaner.php` 已实现，4 种 profile + 白名单过滤 + DOM 遍历）
- [x] 安全过滤辅助函数（`Helper/Security.php`：escHtml / escAttr / escJs / escUrl / csrfField / maskPhone / maskEmail / maskApiKey / isPrivateIp / constantTimeEquals）
- [x] 通用工具函数（`Helper/Utility.php`：buildUrl / humanFileSize / formatDate / timeAgo / slugify / randomFilename / truncate / isAllowedExtension / fileTypeFromMime）
- [ ] 代码审查 + 性能调优
